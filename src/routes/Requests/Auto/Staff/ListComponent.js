...
class ListComponent extends Component {
  ...
  handleCreate = (e) => {
    e.preventDefault()
    this.context.router.push('/requests/auto/staff/add')
  }

  render() {
    const classes = this.props.classes
    let url = `${process.env.REACT_APP_API_HOST}/requests/auto/staff/list`
    ...
    const onRowPrepared = (info) => {
      if (info.rowType === "data") {
        if (info.data.status.id === 'done') {
          ...
          info.rowElement.css('background', '#F44336')
          info.rowElement.css('color', '#FFF')
        }
      }
    }
    return (
